...
export default function DemoProductPage() {
  ...
  const addDemoToCart = () => {
    ...
    setError(false);
    sessionStorage.setItem('rua-demo-product', JSON.stringify({ name: 'Vestido Lino Vera', sku: 'RUA-0001', price: 590000, size: selectedSize, color: 'Crudo', quantity, image: '/brand/vestido-lino-vera.png' }));
    router.push(`/demo-checkout?size=${encodeURIComponent(selectedSize)}&color=Crudo&quantity=${quantity}`);
  };
  ...
  return (
    <main className="min-h-screen bg-[#f7f4ef] text-[#201e1c]">
      ...
      <section className="mx-auto grid w-[94%] max-w-[1500px] gap-10 pb-16 lg:grid-cols-[1.08fr_.92fr] lg:gap-16">
        <div><div className="relative aspect-[4/5] max-h-[760px] overflow-hidden bg-[#e9e1d8]"><Image src="/brand/vestido-lino-vera.png" alt="Vestido Lino Vera de Paradiso" fill priority className="object-cover" sizes="(max-width: 1024px) 100vw, 55vw" /><span className="absolute left-5 top-5 bg-[#201e1c] px-3 py-2 text-[9px] tracking-[0.16em] text-white">-14%</span></div><div className="mt-3 flex gap-3"><button className="relative h-24 w-20 overflow-hidden border border-[#201e1c] bg-[#e9e1d8]"><Image src="/brand/vestido-lino-vera.png" alt="Vista principal" fill className="object-cover" sizes="80px"/></button><div className="flex h-24 w-20 items-center justify-center border border-[#d9d2cb] bg-[#eee9e3] text-center text-[8px] uppercase tracking-[.14em] text-[#817a74]">Más fotos<br/>próximamente</div></div></div>
        ...
        <aside className="self-start lg:sticky lg:top-28">
          ...
          <div className="mt-6 grid grid-cols-[1fr_auto] items-end gap-6"><div><div className="flex items-center justify-between"><p className="text-[10px] uppercase tracking-[0.18em]">Color</p><span className="text-[10px] text-[#6e6965]">Crudo</span></div><div className="mt-3 inline-flex items-center gap-2 border border-[#201e1c] bg-white px-4 py-3 text-xs"><span className="h-3 w-3 rounded-full border border-black/15 bg-[#e9e1d8]"/>Crudo<Check size={12}/></div></div><div><p className="text-[10px] uppercase tracking-[0.18em]">Cantidad</p><div className="mt-3 flex h-[42px] items-center border border-[#d9d2cb] bg-white"><button type="button" aria-label="Disminuir cantidad" onClick={()=>setQuantity(q=>Math.max(1,q-1))} disabled={quantity===1} className="flex h-full w-10 items-center justify-center disabled:opacity-30"><Minus size={13}/></button><span className="min-w-8 text-center text-xs">{quantity}</span><button type="button" aria-label="Aumentar cantidad" onClick={()=>setQuantity(q=>Math.min(12,q+1))} className="flex h-full w-10 items-center justify-center"><Plus size={13}/></button></div></div></div>

          <button type="button" onClick={addDemoToCart} className="mt-8 block w-full bg-[#201e1c] py-5 text-center text-[10px] uppercase tracking-[0.2em] text-white transition hover:bg-[#393532]">Agregar {quantity > 1 ? `${quantity} unidades` : 'al carrito'}</button>
