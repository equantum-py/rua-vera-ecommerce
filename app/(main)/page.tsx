...
export default function Page() {
  return (
    <main className="bg-[#f7f4ef] text-[#201e1c]">
      <section className="grid min-h-[72vh] md:grid-cols-[0.82fr_1.18fr]">
        ...
        <div className="relative min-h-[480px] overflow-hidden bg-[#eee9e3]">
          <Image src="/brand/marcas-con-identidad.svg" alt="RUA Vera — Marcas con identidad" fill priority className="object-cover object-center" sizes="(max-width: 768px) 100vw, 60vw" />
        </div>
      </section>
      ...
      <section className="border-y border-[#cfc7c0] bg-[#eee9e3]">
        <div className="mx-auto grid max-w-[1600px] gap-10 px-[4%] py-16 md:grid-cols-[0.38fr_0.62fr] md:items-center md:py-24">
          ...
          <Link href="/categories" className="group relative block overflow-hidden border border-[#cfc7c0] bg-[#f7f4ef]"><div className="relative aspect-[1.55/1]"><Image src="/brand/colecciones-marcas.svg" alt="Colecciones y marcas que forman RUA Vera" fill className="object-cover transition duration-700 group-hover:scale-[1.015]" sizes="(max-width: 768px) 100vw, 62vw" /></div></Link>
        </div>
      </section>

      <section className="grid md:grid-cols-3">{[['MODA','Prendas y propuestas seleccionadas para descubrir una nueva temporada.'],['LIFESTYLE','Aromas, detalles y piezas para vivir RUA más allá de la moda.'],['OBJETOS & DISEÑO','Objetos especiales que completan una curaduría con identidad.']].map(([title,copy],i) => <Link href="/shop" key={title} className={`flex min-h-[360px] flex-col justify-end border border-[#f7f4ef]/30 p-8 text-white ${i===1?'bg-[#6e6965]':i===2?'bg-[#b4aaa2]':'bg-[#8f8a87]'}`}><span className="text-[9px] uppercase tracking-[0.22em]">0{i+1}</span><h3 className="mt-3 font-serif text-4xl">{title}</h3><p className="mt-4 max-w-sm text-sm leading-6 text-white/70">{copy}</p><span className="mt-7 text-[10px] uppercase tracking-[0.18em]">Comprar →</span></Link>)}</section>
